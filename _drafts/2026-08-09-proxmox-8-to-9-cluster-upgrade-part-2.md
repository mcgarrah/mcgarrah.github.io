...
This is that post. **[Part 3](/proxmox-8-to-9-cluster-upgrade-part-3/)** picks up after this one with the NVIDIA GPU driver work, done deliberately ahead of the OS jump rather than after it; **[Part 4](/proxmox-8-to-9-cluster-upgrade-part-4/)** covers a full *arr media automation stack built on top of that, also ahead of the OS jump; **Part 5** covers the actual Proxmox OS upgrade to Trixie once all three are done.
...
With Ceph's own upgrade plan fully specified, run for real, and one unrelated-but-real redundancy gap closed along the way — **[Part 3](/proxmox-8-to-9-cluster-upgrade-part-3/)** covers the NVIDIA GPU driver work done next, deliberately ahead of the OS jump; **[Part 4](/proxmox-8-to-9-cluster-upgrade-part-4/)** covers a full *arr media automation stack built on top of that, also ahead of the OS jump; **Part 5** covers the actual Proxmox OS upgrade to Trixie once all three are confirmed clean.
