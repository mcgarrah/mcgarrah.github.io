...
| 🗄️ **Ceph Storage** | ceph-osd-moving-disks, ceph-reef-to-squid (superseded) | OSD moves; Reef→Squid upgrade now covered by the `proxmox-8-to-9-cluster-upgrade-part-2` post in the AlteredCarbon series below |
...
| 🏡 **Homepage Dashboard** | homepage-homelab-dashboard, homepage-lxc-standalone-nextjs-bug | Ready (2026-08-12). Two-part series replacing Homarr: (1) the migration itself — VMID/IP/MAC reuse, real YAML config-as-code, the UrBackup widget's self-signed-cert gotcha; (2) a confirmed upstream bug in community-scripts/ProxmoxVE's installer (runs `next start` against a `output: standalone` build, ~7.3x unnecessary disk), tested-and-hardened fix included. Part 2 cross-links back to part 1. Upstream GitHub issue drafted, not yet filed. |
...
- Superseded (2026-08-08) by `2026-08-09-proxmox-8-to-9-cluster-upgrade-part-2.md`, part of the AlteredCarbon PVE8→9 series. This stub's original intent (expand when the upgrade is performed) is now fulfilled there in much greater depth. Archive this stub or fold any remaining unique content into the series.
