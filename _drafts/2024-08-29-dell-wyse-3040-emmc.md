...
I found out that eMMC storage is a different thing entirely. This is especially true when you are booting from it like on the Dell Wyse 3040 which I have several. The goal is to get some status information on the eMMC storage. On other storage, this would mean installing SMART utilities but those do work on eMMC.
...
Another is the low resolution of the values (10% increments), which is bad for benchmarking applications that write small amounts of data. Very long run times would be required to obtain any useful information.
