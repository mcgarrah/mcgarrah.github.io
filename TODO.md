...
- [ ] Add CI check for missing `image:` front matter — New posts should always have an OG image for Article rich results. Add a step to the GitHub Actions build workflow that checks all posts since 2023 have `image:` in their front matter and warns (or fails) if missing. This catches the case where a new post is published without running `bin/generate-og-images.py` first.
...
- [x] Add existing LinkedIn post to `_linkedin/` — Archive the one published LinkedIn post (from the Substack launch period) into the git repository for future reference and drafts site rendering. Companion blog post: `_posts/2024-11-18-isc2-cybersecurity-certification.md`.
- [ ] Re-enable HTML Proofer as blocking (`|| true` → hard fail) — Currently non-blocking because future-dated posts create broken internal links in the production build (which doesn't use `--future`). Once the backlog of scheduled posts clears (after June 29), remove `|| true` from the HTML Proofer step in `.github/workflows/jekyll.yml` to enforce link integrity again. Consider adding `--future` to the CI build or ignoring links to known future-dated slugs.
